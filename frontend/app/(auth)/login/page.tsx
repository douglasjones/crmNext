...
import AlertDialog, { AlertDialogType } from '@/components/ui/AlertDialog';
import LoadingOverlay from '@/components/ui/LoadingOverlay'; // 1. Importar o LoadingOverlay
...
export default function LoginPage() {
  ...
  const [isLoading, setIsLoading] = useState(false); // 2. Renomear estado
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setIsLoading(true); // 5. Usar setIsLoading
    ...
    try {
      ...
      if (responseData && responseData.user) {
        ...
        if (responseData.needsPasswordChange) {
          setIsLoading(false); // Desativa o loading antes de mostrar o alerta
          setAlert({
            isOpen: true,
            type: 'info',
          ...
          router.push('/main');
          // O loading não precisa ser desativado aqui, pois a página será desmontada
        }
      }

    } catch (err: any) {
      setIsLoading(false); // Desativa o loading no erro
      setAlert({ 
        isOpen: true, 
        type: 'error', 
    ...
    // O finally foi removido para um controle mais granular do estado de loading
  };
